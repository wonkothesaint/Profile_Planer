class Debt:
    def __init__(
        self, loan_type, ammount, interest, duration=0, date_start=None, currency="ILS"
    ):
        ...
        if self.loan_type == "Spicher":
            self.monthly_payment = self.ammount * Debt.calc_spicher_monthly_pct(
                self.interest, self.duration
            )
    ...
    def progress_month(self):
        ...
        pay_interest = 0
        if self.loan_type == "Spicher":
            pay_debt = self.monthly_payment * (
                1 / (Debt.calc_spicher_monthly_pct(self.interest, self.months_left))
                - (
                    1
                    / Debt.calc_spicher_monthly_pct(self.interest, self.months_left - 1)
                )
            )
