...
def assets_excel_to_json(user):
    ...
    while not sheet["A" + str(i)].value is None:
        ...
        data[sheet["A" + str(i)].value] = {
            "value": float(sheet["B" + str(i)].value[1:].replace(",", "")),
            "deposit_fee_pct": reduce_float_percision(sheet["D" + str(i)].value, 4),
            "management_fees_yearly_pct": reduce_float_percision(
                sheet["F" + str(i)].value, 4
            ),
            "yield_yearly_pct": reduce_float_percision(sheet["H" + str(i)].value, 4),
            "dividends_yield_yearly_pct": reduce_float_percision(
                sheet["J" + str(i)].value, 4
            ),
